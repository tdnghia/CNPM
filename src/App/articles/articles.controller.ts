import { Body, Controller, Delete, HttpException, HttpStatus, InternalServerErrorException, Param, Put } from '@nestjs/common';
...
export class ArticlesController extends BaseController<Article> {
    ...
    async createOne(@ParsedRequest() req: CrudRequest, @ParsedBody() dto: Article, @UserSession() user:any) {
      try {
        const findUser = await this.userRepository.findOne({id: user.users.id});
        const findCategory = await this.categoryRepository.findOne({ id: dto.categoryId });
    ...
    @Put('updateOne/:slug')
    async updateUser(@Body() dto: Partial<Article>, @Param('slug') slugParam: string) {
      try {
